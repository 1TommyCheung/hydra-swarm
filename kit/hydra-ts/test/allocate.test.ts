...
function setupFixture(): FixturePaths {
  ...
  mkdirSync(join(repoRoot, 'hydra', 'profiles'), { recursive: true });
...
function writeSeed(
  ...
  writeFileSync(join(repoRoot, 'hydra', 'profiles', filename[vendor]), content, 'utf8');
}
...
describe('allocate', () => {
  ...
  it('throws when no eligible vendor remains after exclusion', () => {
    const { repoRoot, stateRoot } = setupFixture();
    writeSeed(repoRoot, 'kimi', 'cost_hint: "0.95/4.00"\n');
    assert.throws(
      () => allocate('visual_debugging', 'screenshot', 'medium', 'kimi', { repoRoot, stateRoot }),
  ...
  it('returns only kimi for visual_debugging role', () => {
    const { repoRoot, stateRoot } = setupFixture();
    writeSeed(repoRoot, 'kimi', 'cost_hint: "0.95/4.00"\n');
    const result = allocate('visual_debugging', 'screenshot', 'medium', '', { repoRoot, stateRoot });
  ...
  it('excludes opencode from implementer candidates', () => {
    const { repoRoot, stateRoot } = setupFixture();
    writeSeed(repoRoot, 'claude', 'cost_hint: subscription\n');
    writeSeed(repoRoot, 'codex', 'cost_hint: "5.0/30.0"\n');
    writeSeed(repoRoot, 'kimi', 'cost_hint: "0.95/4.00"\n');
    const result = allocate('implementer', 'feature', 'medium', '', { repoRoot, stateRoot });
  ...
  it('uses zero for missing measured stats when n>=8', () => {
    ...
    const result = allocate('integrator', 'feature', 'medium', '', { repoRoot, stateRoot });
  ...
  it('falls back to seeded priors when measured n<8', () => {
    const { repoRoot, stateRoot } = setupFixture();
    writeSeed(repoRoot, 'claude', 'seeded_strengths:\n  - claim: implementation\n    cost_hint: subscription\n');
    writeSeed(repoRoot, 'codex', 'cost_hint: "5.0/30.0"\n');
    writeSeed(repoRoot, 'kimi', 'cost_hint: "0.95/4.00"\n');
  ...
  it('uses seed relevance to break ties', () => {
    const { repoRoot, stateRoot } = setupFixture();
    writeSeed(repoRoot, 'claude', 'seeded_strengths:\n  - claim: implementation\ncost_hint: subscription\n');
    writeSeed(repoRoot, 'codex', 'seeded_strengths:\n  - claim: review\ncost_hint: "5.0/30.0"\n');
    writeSeed(repoRoot, 'kimi', 'cost_hint: "0.95/4.00"\n');
  ...
  it('honours --exclude-vendor', () => {
    ...
    const result = allocate('implementer', 'feature', 'medium', 'claude', { repoRoot, stateRoot });
  ...
  it('defaults risk to medium', () => {
    const { repoRoot, stateRoot } = setupFixture();
    writeSeed(repoRoot, 'claude', 'cost_hint: subscription\n');
    const result = allocate('integrator', 'merge', '', '', { repoRoot, stateRoot });
  ...
  it('passes through risk and excluded null when empty', () => {
    const { repoRoot, stateRoot } = setupFixture();
    writeSeed(repoRoot, 'claude', 'cost_hint: subscription\n');
    const result = allocate('integrator', 'merge', 'high', '', { repoRoot, stateRoot });
  ...
  it('reflects reviewer role ordering and excludes kimi', () => {
    const { repoRoot, stateRoot } = setupFixture();
    writeSeed(repoRoot, 'codex', 'cost_hint: "5.0/30.0"\n');
    writeSeed(repoRoot, 'opencode', 'cost_hint: free_tier\n');
    writeSeed(repoRoot, 'claude', 'cost_hint: subscription\n');
    const result = allocate('reviewer', 'audit', 'medium', '', { repoRoot, stateRoot });
  ...
  it('uses the claude and codex default for an unknown role', () => {
    ...
    const result = allocate('unknown', 'feature', 'medium', '', { repoRoot, stateRoot });
  ...
  it('preserves grep -E behavior for regex metacharacters in task types', () => {
    ...
    const result = allocate('integrator', 'c++_build', 'medium', '', { repoRoot, stateRoot });
  ...
  it('throws when excluding the only eligible vendor', () => {
    ...
    writeSeed(repoRoot, 'kimi', 'cost_hint: "0.95/4.00"\n');
    assert.throws(
      () => allocate('visual_debugging', 'ui', 'medium', 'kimi', { repoRoot, stateRoot }),
      /no eligible vendor for role=visual_debugging/,
    );
  });
});
