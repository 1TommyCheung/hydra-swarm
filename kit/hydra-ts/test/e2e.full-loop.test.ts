...
// with the REAL TypeScript stub vendor adapter (kit/hydra-ts/src/adapter-stub.ts).
// Zero real vendor API calls, zero shell adapters.
//
// Modeled on e2e.smoke.test.ts (one shared state root + scratch git repo, no
// mocks) but goes further: dispatch() spawns the REAL adapter-stub.ts through
// `node --experimental-strip-types` as a child process (legitimate — the stub
// is offline and fast, so dispatch's process-launch, keep-alive timeout, and
// exit-sentinel machinery gets real coverage). The resulting inbox drop is then
// promoted, squashed, and integrated through the real TS modules, verifying a
// byte-for-byte identical tree to the stub's commit. Negative cases exercise
// distinct no_commit and not_completed rejection gates using drops whose Git
// fields came from the scratch repo.
//
// The shell adapter lane (kit/hydra/adapters/stub.sh) was retired in run 0045
// (docs/bash-lane-retirement-plan.md); this suite exercises the TypeScript
// adapter-stub exclusively and asserts the dispatch spawn is a Node process
// running adapter-stub.ts (never a shell adapter).
...
async function dispatchStub(
  ...
  // adapter-stub.ts` (ZERO shell processes for the adapter step). The recorder
  ...
  for (const entry of spawned) {
    ...
    assert.doesNotMatch(
      argv,
      /adapters[\\/]stub\.sh/,
      'TS adapter step must not reference the retired shell stub.sh',
    );
  }
...
describe('e2e full-loop: dispatch -> promote -> squash -> integrate (TypeScript stub adapter)', { concurrency: 1 }, () => {
  before(() => {
    saved = {
  ...
  it('happy path: node-spawned adapter-stub.ts -> promote -> squash -> integrate with identical tree (zero bash)', async () => {
    const ctx = setupRun('happy-ts', RUN_HAPPY);
    ...
    const intHead = await integrate(ctx.runId, [ctx.taskId], {
      cwd: ctx.worktree,
      stateRoot: STATE,
  ...
  it('negative paths: node-spawned no_commit and fail hit distinct rejection gates', async () => {
    const noCommit = setupRun('no-commit-ts', RUN_NO_COMMIT);
    ...
    const failed = setupRun('failed-after-commit-ts', RUN_FAIL);
