...
# plugin's version into the version-keyed cache hydra_resolve_bin probes.
#
...
#   2. the binary's own `version --json` must equal .claude-plugin/plugin.json
...
url_base="${base_url}/v${version}"

tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/hydra-fetch-bin.XXXXXX")"
trap 'rm -rf "$tmp_dir"' EXIT

printf 'hydra: fetching %s (v%s, %s)\n' "$bin_asset" "$version" "$url_base" >&2
curl -fsSL "${curl_proto[@]}" -o "$tmp_dir/$manifest_asset" "$url_base/$manifest_asset" \
  || hydra_die "manifest download failed: $url_base/$manifest_asset (is release v$version published?)"
...
[ "$reported" = "$version" ] \
  || hydra_die "binary self-reports version '${reported:-unreadable}', expected '$version' — refusing to install"
...
printf 'hydra: installed %s/%s (sha256 verified, self-reported v%s)\n' "$install_dir" "$bin_asset" "$version"
