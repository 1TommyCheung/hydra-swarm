...
if [ "${HYDRA_HARNESS:-ts}" = "bin" ] && HYDRA_BIN_PATH="$(hydra_resolve_bin)"; then exec env -u BUN_BE_BUN "$HYDRA_BIN_PATH" create-worktree "$@"; fi
HYDRA_NODE="$(hydra_resolve_node)"; exec "$HYDRA_NODE" --experimental-strip-types "$SELF_DIR/../../hydra-ts/src/cli.ts" create-worktree "$@"
fi
...
bootstrap_status=ok
bootstrap_policy="$repo_root/kit/hydra/policies/bootstrap.yaml"  # per-project config; keep in parity with hydra-ts/src/create-worktree.ts
