...
The compiled Bun binary is the default runtime as of the 2026-07-16 cutover: unset `HYDRA_HARNESS` prefers `bin`, falling back SILENTLY to the TypeScript/Node source lane (`ts`) only when no binary is resolvable yet (no `HYDRA_BIN`, no `npm run build:bin` output) — a fresh checkout with nothing pre-built still works out of the box. Continue calling the unchanged `kit/hydra/scripts/*.sh` entry points; each is a small launcher, unchanged from your perspective — the runtime selection happens inside `hydra_launch()` in `lib.sh`. The Bash implementation lane was **retired** in run 0045 (dev note `docs/dev-notes/bash-lane-retirement-plan.md`, machine-local — recoverable from git history pre-split): `HYDRA_HARNESS=bash` and `HYDRA_ADAPTER_RUNTIME=bash` now fail loudly with an explicit retirement error and do **not** silently coerce to `ts`.
...
6. Review promoted candidates: cross-vendor by convention — Codex reviews Claude and vice versa. Dispatch a cross-vendor review run (e.g. Codex reviewing a Kimi-authored candidate). Record the verdict with `bash ${CLAUDE_PLUGIN_ROOT}/kit/hydra/scripts/record-review.sh <run-id> <task> <verdict.json>` — it publishes an append-only generation under `authoritative/reviews/<task>/`; a finished reviewer process is telemetry, never a verdict. Only a recorded `accept` lets the candidate proceed. On `revise`, amend the spec (`amend-task.sh`) and re-dispatch — the worker receives the recorded verdicts as a file-first evidence bundle (see `${CLAUDE_PLUGIN_ROOT}/docs/dispatch-modes.md` §3).
...
10. Close out (after the human merges): `bash ${CLAUDE_PLUGIN_ROOT}/kit/hydra/scripts/run-log.sh <run-id>` writes the durable lifecycle audit to `docs/hydra-dev-logs/run-<id>.md` (override with `--out` or `HYDRA_DEV_LOG_DIR`), then `bash ${CLAUDE_PLUGIN_ROOT}/kit/hydra/scripts/gc.sh --apply --keep-last 3` reaps worktrees+branches whose integration the ledger PROVES (recorded SHA reachable from the default branch, same-evidence-chain pairing, revalidated before each destructive op). Document, then delete: the run log + ledger + squash commits carry the audit trail the worktree used to. gc is fail-closed — worktrees integrated outside `integrate.sh` (e.g. a GitHub PR squash-merge) can never be proven and stay untouched; remove those manually with `git worktree remove` once their PR is merged. Default is dry-run; `--apply` is required to mutate; in-flight/failed/unproven worktrees are never touched.
...
- **Read-only vendor consultation** (describe/analyze/advise, one-off cross-vendor question) → `bash ${CLAUDE_PLUGIN_ROOT}/kit/hydra/scripts/review-dispatch.sh <run_id> <review_id> <vendor> <prompt_file> --task <task_id>`. `--task` is REQUIRED (issue #32): it stamps explicit `task_id` provenance on both lifecycle events, and task identity is never inferred from the review id's naming (for a consultation not tied to a real task, pass the task the question is about, or a dedicated consultation task id). Despite the name it is the general read-only vendor lane, not just for candidate reviews: it hosts every vendor — including OpenCode — directly in the same herdr pane (no decoupled monitor pane; that split-pane mechanism is a full-worker-dispatch-only behavior), appends `review_started`/`review_completed` to the run ledger, and captures the raw session to `runs/run-<id>/sessions/<review_id>.<vendor>.raw`. It needs a run to attach to — reuse the active run's id, or `run-init.sh` a consultation run first.
...
- [references/worker-devenv.md](references/worker-devenv.md) — what the dispatcher's worker dev-environment preflight (`prepareWorkerEnv()`) guarantees before spawn (network domains, package-manager store/cache dirs, toolchain verification), what a task spec should still declare via `network_domains`, and why a mid-task environment failure should be filed as a preflight gap rather than worked around with symlinks.
...
- [${CLAUDE_PLUGIN_ROOT}/docs/dispatch-modes.md](${CLAUDE_PLUGIN_ROOT}/docs/dispatch-modes.md) — the authoritative per-mode reference: full worker dispatch, read-only review dispatch, amendment/revise, promotion, squash + integration, and the operational commands — command, state written, worker visibility, observability, termination, and failure modes for each.
