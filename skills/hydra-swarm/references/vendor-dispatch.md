...
`review-dispatch.sh` is the read-only vendor lane for anything that isn't a full task dispatch: one-off cross-vendor reviews AND read-only consultations (describe/analyze/advise prompts). It is the required path for those — never invoke a vendor CLI as a raw shell command, even read-only; see SKILL.md §"Vendor CLIs are never invoked directly". The prompt file is arbitrary text, so a consultation is just a review-dispatch whose prompt asks a question instead of requesting a verdict.

The full invocation is `review-dispatch.sh <run_id> <review_id> <vendor> <prompt_file> --task <task_id> [--image PATH]` (issue #32). `--task` is required and must be a canonical task id (1-64 chars of `[a-z0-9-]`, no leading/trailing hyphen); it is stamped as explicit `task_id` on both `review_started` and `review_completed` — task identity is never inferred from `review_id`, which stays an opaque session-artifact label. The argv contract is strict: exactly one `--task`, at most one `--image` (either order, before/between/after the operands), and unknown flags, extra operands, duplicates or missing flag values all fail loudly BEFORE any ledger append or session file is created. Note a completed reviewer process is telemetry only — a verdict exists only once `record-review.sh` publishes it as an append-only generation at `authoritative/reviews/<task>/<seq>-<reviewed_head>.json`.

`review-dispatch.sh`/`review-dispatch.ts` used to redirect the entire vendor command's output straight to a file with nothing tailed into the pane, so a genuinely active Codex or Kimi review looked identical to a hung one: banner only, then silence until it finished. This is now fixed: Codex/Kimi review panes live-tail the same way worker panes do, reusing the review's own `.raw` NDJSON capture as the source (no separate capture file needed). Claude/OpenCode reviewer panes are unchanged (banner only, matching their worker-dispatch behavior).
