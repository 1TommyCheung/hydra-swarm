...
under `kit/hydra/adapters/` and
`kit/hydra-ts/src/adapter-{claude,codex,opencode,kimi}.ts`. Dispatch uses the
TypeScript adapters by default through the normal `kit/hydra/scripts/*.sh` command
